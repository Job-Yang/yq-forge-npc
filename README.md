# YQ 铁匠铺 · 打工人语音面板 v2.7.2

一个像素风可交互 NPC 语音面板：点击人族农民 / 兽族苦工，他们会冒像素台词框 + 播放对应 War3 语音；工程师 YQ 不说话，只会定时冒泡。三个 NPC 都会在地面上自己瞎逛、自己挖矿。

**v2.7.2 动画修复**：
- **YQ 4 帧真·独立动画**：idle/talk/walk1/walk2 重画为 4 张不同姿势（之前是同一张图复用，所以 YQ 站着不动也不抬腿）。padding 到同一画布、底部对齐，切帧时脚不漂浮

**v2.7.1 视觉修复**：
- **豆包脸重画**：3D 动漫女孩风像素化（棕色 bob 头 + 大眼），贴近真实豆包形象
- **金矿放大 110px → 220px**：终于比 NPC 还大,占地面右侧 C 位
- **YQ 形象重画**：戴眼镜 + 蓝色帽衫 + 棕色围裙 + 手捧 MacBook,对齐主页 banner 的工程师设定

**v2.7 玩法升级**：
- **YQ 形象重画**：从围裙工程师改为「盾甲码农」—— 一手锤子一手 iPad，跟铁匠铺主页 banner 的硬核基调对齐
- **金矿系统**：右下角金矿，苦工/农民会自主跑过去挖矿，每挖 3 镐 +10 金币，左上角实时计数；攒到 100 解锁第 4 个 NPC（v2.8 见）
- **气泡停留时长动态化**：基础 3s + 每字 0.18s 封顶 9s，长台词再也不会一秒就消失
- **深夜彩蛋（23:00-03:00）**：背景压暗，YQ 自动循环说「你也没睡?」「这点该 commit 了」「别卷了」等
- **豆包偷窥**：每 40-90s 随机从屏幕边缘（左/右/上）冒出豆包脸偷看，偶尔配 YQ 反应「豆包又来了」「别看我」
- 金币状态写 localStorage，刷新页面也在

**v2.6 加载性能优化**：PNG 6.6MB→2.0MB (省 70%)、WAV→OGG 2.8MB→175KB (省 94%)、整包 9.4MB→2.6MB；加 loading 蒙层关键资源优先放行（首屏 < 1s）
